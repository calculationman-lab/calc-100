...
test("数字入力で敵上演出発火",()=>{const h=createHarness();h.run('kind="normal";v="";key("7")');assert(h.nodes.get("battleFeedback").classList.contains("fire"),"数字発火")});
...
if(failures.length){console.error(`Slash v15 tests: ${passed}/${total} passed`);failures.forEach(f=>console.error(`FAIL ${f}`));process.exit(1)}
console.log(`Slash v15 tests: ${passed}/${total} passed`);
